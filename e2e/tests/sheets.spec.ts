...
test.describe('Sheets (Paywall / Cancel / EmailPreview / Feedback / WhyVaulted)', () => {
  ...
  test('Paywall: triggered from Settings demo, 5-row table + 7-day trial CTA', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /Mafia Plus extends your Vault\./ })).toBeVisible();
    // 5 comparison rows.
    await expect(page.getByText('Vault retention')).toBeVisible();
    await expect(page.getByText('Surfaces connected')).toBeVisible();
    await expect(page.getByText('Restores per month')).toBeVisible();
    await expect(page.getByText('Deep restore (snapshots)')).toBeVisible();
    await expect(page.getByText('Cross-surface coherence')).toBeVisible();
    // CTA + cancel-anytime footer.
    await expect(page.getByRole('button', { name: /Try free for 7 days/ })).toBeVisible();
    await expect(page.getByText(/Cancel anytime in 2 taps/)).toBeVisible();
  });
  ...
  test('Why Vaulted: long-press on a Vault item exposes the sheet', async ({ page }) => {
    ...
    // Long-press is implemented via contextmenu in useLongPress; we
    // dispatch a contextmenu event on the first item row.
    // The row container is the parent of the thumbnail. We use a stable
    // selector: the item title text "LinkedIn digest × 47" (data.ts v2).
    const itemRow = page.locator('div').filter({ hasText: /LinkedIn digest/ }).first();
